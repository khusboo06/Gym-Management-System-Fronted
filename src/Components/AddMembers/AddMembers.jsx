    ...
    await axios
      ...
      .then((res) => {
        ...
        if (res.data.membership.length === 0) {
          ...
          const firstId = res.data.membership[0]._id
          setSelectedOption(firstId)
          setInputField({ ...inputField, membership: firstId })
        }
      })
      .catch((err) => {
        console.log(err)
    ...
    <div className='text-black'>
      <div className='grid gap-5 grid-cols-1 sm:grid-cols-2 text-lg'>
        ...
        <select
          value={selectedOption}
          onChange={handleOnChangeSelect}
          className='border-1 w-full sm:w-[90%] h-10 pt-2 pb-2 border-slate-400 rounded-md'
        >
          {
        ...
        <input
          ...
          type="file"
          className='border-1 w-full sm:w-[90%] h-10 pl-2 pt-2 pb-2 border-slate-400 rounded-md'
        />

        <div className='w-[80px] h-[80px]'>
          <img src={inputField.profilePic} className='w-full h-full rounded-full' alt="" />
          {loaderImage && (
            <Stack sx={{ width: '80px' }} spacing={2}>
              <LinearProgress color="secondary" />
            </Stack>
          )}
        </div>
